...
static SDL_TextureID texture;
//static GLuint tex;
static GLfloat points[8];
static GLfloat texturePoints[8];
...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
		...
		GLfloat blank = (is->height - is->width * aspect_ratio) / 2;
		points[0] = blank;
		points[1] = 0;
		points[2] = blank;
		points[3] = is->height;
		points[4] = is->width-blank;
		points[5] = 0;
		points[6] = is->width-blank;
		points[7] = is->height;
		...
		if (width > is->width) {
			...
			points[1] = blank;
			points[2] = 0;
			points[3] = is->height-blank;
			points[4] = is->width;
			points[5] = blank;
			points[6] = is->width;
			points[7] = is->height-blank;
		}
		x = (is->width - width) >> 1;
		...
		//SDL_TextureID tex = SDL_CreateTextureFromSurface(SDL_PIXELFORMAT_RGB888, vp->bmp);
		/*if (!texture) 
			fprintf(stderr, "SDL: could not create texture : %s\n",SDL_GetError());*/
		//rect.x = 0; rect.y = 0; rect.w = vp->bmp->w; rect.h = vp->bmp->h;
		//SDL_RenderCopy(tex, &srcrect, &rect);
		SDL_RenderPresent();
...
#if 1
      //  fill_rectangle(screen,
       //                is->xleft, is->ytop, is->width, is->height,  SDL_MapRGB(screen->format, 0x00, 0x00, 0x00));
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts)
{
	...
	if (vp->bmp) {
		...
		dst_pix_fmt = PIX_FMT_RGB555LE;
		avpicture_alloc(&pict, dst_pix_fmt,is->video_st->codec->width, is->video_st->codec->height);
		sws_flags = av_get_int(sws_opts, "sws_flags", NULL);
		img_convert_ctx = sws_getCachedContext(img_convert_ctx, is->video_st->codec->width, is->video_st->codec->height, is->video_st->codec->pix_fmt, is->video_st->codec->width, is->video_st->codec->height, dst_pix_fmt, sws_flags, NULL, NULL, NULL);
		if (img_convert_ctx == NULL) {
			fprintf(stderr, "Cannot initialize the conversion context\n");
			exit(1);
		}
		sws_scale(img_convert_ctx, src_frame->data, src_frame->linesize, 0, is->video_st->codec->height, pict.data, pict.linesize);
		vp->bmp->pixels = pict.data[0];		
		SDL_UnlockSurface(vp->bmp);
		vp->pts = pts;
		...
		gettimeofday(&fin, &tz);
		duree=(fin.tv_sec-depart.tv_sec) * 1000000L + (fin.tv_usec-depart.tv_usec);  
		printf("Durée sws_scale %ld microsecondes\n", duree);
    }
    return 0;
}
    ...
    for(;;) {
        ...
        switch(event.type) {
			...
			case SDL_MOUSEBUTTONUP:
                ...
                selected = SDL_SelectMouse(which);
				...
				if (cur_stream) {
					...
					tmm = (tns%3600)/60;
					tss = (tns%60);
					frac = (double)x/(double)cur_stream->width;
					ns = frac*tns;
					hh = ns/3600;
					mm = (ns%3600)/60;
					ss = (ns%60);
...
int main(int argc, char **argv)
{
	...
	for(i=0; i<CODEC_TYPE_NB; i++){
		//jj		avcodec_opts[i]= avcodec_alloc_context2(i);
		avctx_opts[i]= avcodec_alloc_context2(i);
	}
	//jj	avformat_opts = avformat_alloc_context();
...
//	input_filename = "test04.mp4";
	...
	/*glEnable( GL_BLEND );
	 ...
	 //Landscape Mode - Doesn't work yet
	 //glOrthof(0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, -1.0f, 1.0f);	
	 //glRotatef(-90,0,0,1);
	 //glTranslatef(-SCREEN_HEIGHT, 0, 0);
	...
	/*glGenFramebuffersOES(1, &framebuffer);
	 ...
	 if (err = glGetError())
	 fprintf(stderr,"Error: Could not bind render buffer to frame buffer: %d", err);
	 glGenTextures(1, &tex);
	 if (err = glGetError())
	 fprintf(stderr,"Error: Could not generate texture: %d", err);
